...
async function seedMuscleGroups() {
  ...
  for (const muscleGroup of muscleGroups) {
    await pool.query(
      "INSERT INTO muscle_groups (name, description) VALUES ($1, $2) ON CONFLICT (name) DO NOTHING",
      [muscleGroup.name, muscleGroup.description]
    );
  }
}
...
async function seedExercises() {
  ...
  const categories = categoryResult.rows.reduce(
    (acc: Record<string, number>, curr: { name: string; id: number }) => {
  ...
  const muscleGroups = muscleGroupResult.rows.reduce(
    (acc: Record<string, number>, curr: { name: string; id: number }) => {
      acc[curr.name] = curr.id;
      return acc;
    },
  ...
  const exercises = [
    ...
    },
    {
      name: "Squat",
      description:
        "Lower your body by bending your knees and then stand back up",
      category_id: categories["Strength"],
  ...
  for (const exercise of exercises) {
    await pool.query(
      `INSERT INTO exercises (name, description, category_id, muscle_group_id, difficulty)
...
async function seedWorkoutPlans(userId: number) {
  ...
  for (const plan of workoutPlans) {
    ...
    await pool.query(
      ...
      [planId, userId, scheduledDate.toISOString(), "scheduled"]
    );
  }
}
